...
#define OBS_MESSAGE 			 "$PATCN34%c"
#define VOLUME_MESSAGE   		 "$PATCN73%c"

#define ACTIVE_FREQ_IDENTFIER 		"27"
#define STANDBY_FREQ_IDENTFIER 		"28"
#define OBS_IDENTIFIER				"34"
#define VOLUME_IDENTIFER		    "73"
...
//			  rxcountcdu++;
			  ...
			  if (rxbuffcdu[0] == '\n')
			  {
				  ...
				  rxcountcdu = 0;
				  if(rxmsgcdu[0] == '$' && rxmsgcdu[1] == 'P' && rxmsgcdu[2] == 'A' && rxmsgcdu[3] == 'T' && rxmsgcdu[4] == 'C')
				  {
					  rxfreecdu = true;
				  }
			  }
		  ...
		  if(rxfreecdu)
		  {
			  ...
			  if(rxmsgcdu[5] == RCU_DESIG)
			  {
				  ...
				  else if(Test_Pattern(&rxmsgcdu[6], VOLUME_IDENTIFER))// volume level
				  {
					  int val;
