...
#define PRE_PRO_8 "P8"
#define PRE_PRO_9 "P9"
#define PRE_PRO_10 "P10"
#define PRE_PRO_11 "P11"
#define PRE_PRO_12 "P12"
#define PRE_PRO_13 "P13"
#define PRE_PRO_14 "P14"
#define PRE_PRO_15 "P15"
#define PRE_PRO_16 "P16"
#define PRE_PRO_17 "P17"
#define PRE_PRO_18 "P18"
#define PRE_PRO_19 "P19"
...
const char *screenNames[TOTAL_SCREENS] = {
    ...
    "HF_P3_PROG",     // HF_P3_PROG_SCREEN
    "HF_P4_PROG",     // HF_P4_PROG_SCREEN
    "HF_P5_PROG",     // HF_P5_PROG_SCREEN
...
void update_TACAN_frequency_Screen(TFT_Screen screen);
void update_TACAN_channael_Screen(TFT_Screen screen);
...
void tft_lcd_send_command_float(const char *objectName, const char *command, float value,
                                const char *identifier);
void tft_lcd_send_command_channel(const char *objectName, const char *command, char *value,
                                  const char *identifier);
...
void hf_p3_screen_handler(uint8_t key);
void hf_p4_screen_handler(uint8_t key);
void hf_p5_screen_handler(uint8_t key);
void hf_p6_screen_handler(uint8_t key);

void hf_p1_prog_screen_handler(uint8_t key);
void hf_p2_prog_screen_handler(uint8_t key);

void hf_p3_prog_screen_handler(uint8_t key);
void hf_p4_prog_screen_handler(uint8_t key);
void hf_p5_prog_screen_handler(uint8_t key);
...
ScreenHandler screen_handlers[] = {
    ...
    hf_p3_screen_handler,
    hf_p4_screen_handler,
    hf_p5_screen_handler,
    hf_p6_screen_handler,
    hf_p1_prog_screen_handler,
    hf_p2_prog_screen_handler,
    hf_p3_prog_screen_handler,
    hf_p4_prog_screen_handler,
    hf_p5_prog_screen_handler,
...
void update_Counters(void) {
  if (cdu_parameters.NAV_message_counter < 50000)
    cdu_parameters.NAV_message_counter += 50;  // Counts the time after the last nav health message
  if (cdu_parameters.TACAN_message_counter < 50000)
    cdu_parameters.TACAN_message_counter +=
        50;  // Counts the time after the last tacan health message
  if (cdu_parameters.HF_message_counter < 50000)
    cdu_parameters.HF_message_counter += 50;  // Counts the time after the last hf health message
  if (cdu_parameters.UHF_message_counter < 50000)
    cdu_parameters.UHF_message_counter += 50;  // Counts the time after the last uhf health message
  if (cdu_parameters.VHF_message_counter < 50000)
    cdu_parameters.VHF_message_counter += 50;  // Counts the time after the last vhf health message
 ...
 * the `tft_lcd` function to handle the received key. The task yields
 * periodically to allow other tasks to run.
 *
...
void tft_lcd_thread(void *pvParameters) {
  ...
  cdu_parameters.TACAN_message_counter =
      5050;                                   // Counts the time after the last tacan health message
  ...
  /* Infinite loop */
  // 	start_timer_cursor(); 	// for the testing purpose
  // 	HAL_GPIO_EXTI_Callback(LEFT_SW3);
  for (;;) {
    if (xQueueReceive(xKeyQueue, &receivedKey, (TickType_t)20) == pdTRUE) {
      if (xSemaphoreTake(xFlashMutex, portMAX_DELAY) == pdTRUE) {
        ...
        } else if (receivedKey == BLINK_CURSOR) {
          cursor_blink();
        } else if (receivedKey == REVERT_IN) {
          stop_timer_cursor();
          revert_screen_entry(current_screen, prefix_index);
          Input_mode = false;
        } else if ((receivedKey == SWI_CHANNEL) || (receivedKey == SWI_FREQUENCY)) {
          if (receivedKey == SWI_CHANNEL) {
            update_TACAN_channael_Screen(current_screen);
          } else if (receivedKey == SWI_FREQUENCY) {
            update_TACAN_frequency_Screen(current_screen);
          }
        } else {
          tft_lcd(receivedKey);
        }
 ...
 *
 * Description:
 * 				This function is used to send data and commands
 * to specified object on the TFT display.
 *
 */
void tft_lcd_send_command(const char *objectName, const char *command, const char *value) {
  ...
  //    snprintf(fullCommand, sizeof(fullCommand), "%s.%s.%s=%s",
  //    screenNames[screen_idx], objectName, command, value);

  // Append the 0xFF terminator required by Nextion
  char endCommand[] = {0xFF, 0xFF, 0xFF};

  // Send the command via UART
  HAL_UART_Transmit(&TFT_UART, (uint8_t *)fullCommand, strlen(fullCommand), 500);
  HAL_UART_Transmit(&TFT_UART, (uint8_t *)endCommand, 3,
                    500);  // Send the end command
 ...
 *
 * Description:
 * 				This function is used to convert the decimal
 * values to string for sending it to the TFT display.
 *
 */
void tft_lcd_send_command_int(const char *objectName, const char *command, uint32_t value) {
  char valueStr[20];
  snprintf(valueStr, sizeof(valueStr), "%ld",
           value);  // Convert integer value to string
 ...
 *
 */
void tft_lcd_send_command_(const char *objectName, const char *command, uint16_t value,
                           const char *identifier) {
  char valueStr[20];

  if (identifier == NULL || *identifier == '\0')  // Check if identifier is NULL or empty
  {
    snprintf(valueStr, sizeof(valueStr), "Vol %02d",
             value);  // Convert float value to string without identifier
  } else {
    snprintf(valueStr, sizeof(valueStr), "%s %04d", identifier,
             value);  // Convert with identifier
  }

  tft_lcd_send_command_text(objectName, command,
                            valueStr);  // Send formatted string
}
...
void tft_lcd_send_command_float(const char *objectName, const char *command, float value,
  ...
  if (identifier == NULL || *identifier == '\0')  // Check if identifier is NULL or empty
  {
    ...
    if ((current_screen == TACAN_P1_FQ_SCREEN) || (current_screen == TACAN_P1_FQ_PROG_SCREEN) ||
        (current_screen == TACAN_P2_FQ_SCREEN) || (current_screen == TACAN_P2_FQ_PROG_SCREEN)) {
      snprintf(valueStr, sizeof(valueStr), "%s %.2f", identifier,
               value);  // Convert with identifier
    } else if ((current_screen == HF_P1_PROG_SCREEN) || (current_screen == HF_P1_SCREEN) ||
               (current_screen == HF_P2_PROG_SCREEN) || (current_screen == HF_P2_SCREEN)) {
      snprintf(valueStr, sizeof(valueStr), "%s %.4f", identifier,
               value);  // Convert with identifier
    } else {
      snprintf(valueStr, sizeof(valueStr), "%s %.3f", identifier,
               value);  // Convert with identifier
    }
  }
...
void cursor_blink(void) {
  // Remove Cursor
  if ((!cursor_show) && (input_index < MAX_STANDBY_FREQ_LEN)) {
    input[input_index] = ' ';
    if (prefix_index == 0) {
    ...
    cursor_show = true;
    input[input_index] = 0;  // set back to null
  }
  // Show Cursor
  else if ((cursor_show) && (input_index < MAX_STANDBY_FREQ_LEN)) {
    input[input_index] = '_';
    if (prefix_index == 0) {
      tft_lcd_send_command_text("t1", TEXT, input);
    } else {
 ...
 *
 * Description:
 * 				This function is used to send the TACAN channel
 * data to the TFT display.
 *
 */
void tft_lcd_send_command_channel(const char *objectName, const char *command, char *value,
                                  const char *identifier) {
  ...
  if (identifier == NULL || *identifier == '\0')  // Check if identifier is NULL or empty
  {
    ...
    snprintf(valueStr, sizeof(valueStr), "%s %s", identifier,
             value);  // Convert with identifier
  }
 ...
 *
 * Description:
 * 				This function is used to change the TFT displays
 * page
 *
 */
void tft_lcd_change_screen(TFT_Screen screen) {
  ...
  HAL_UART_Transmit(&TFT_UART, (uint8_t *)command, strlen(command), 500);
  HAL_UART_Transmit(&TFT_UART, (uint8_t *)endCommand, 3, 500);
...
void tft_lcd_Home(int mode) {
  //	tft_lcd_change_screen(NAV_P1_SCREEN);	//Set the screen to Home Screeen
  // At the Initialization
  if (mode == 1) tft_lcd_change_screen(HOME_SCREEN);

  vTaskDelay(pdMS_TO_TICKS(50));  // Delay for 50 milliseconds

  // Check which units are enabled/operational if displayed on Home Screen
  if (current_screen == HOME_SCREEN) {
    if (cdu_parameters.NAV_message_counter > 3000)
      tft_lcd_send_command_int("t1", "pco", RED);
    else
    ...
    if (cdu_parameters.VHF_message_counter > 3000)
 ...
 *
 * Description:
 * 				This function is used to navigate on the HOME
 * page according to the Keyboards input.
 *
 */
void home_screen_handler(uint8_t key) {
  ...
  switch (key) {
    case LEFT_SW1:
      if (cdu_parameters.NAV_message_counter > 3000) {
        ...
        tft_lcd_send_command_text("t9", "txt", "ERR05");
        tft_lcd_send_command_text("t10", "txt", " ");
      } else {
 ...
 *
 * Description:
 * 				This function is used to navigate on the NAV
 * Page 1 according to the Keyboards input.
 *
 */
void nav_p1_screen_handler(uint8_t key) {
    ...
    switch (key) {
      ...
      case ADD:
        if (cdu_parameters.volume_nav < (VOLUME_MAX + 1)) {
          ...
          send_to_uart_queue_volume(NAV, VOLUME, cdu_parameters.volume_nav);
        }
        break;
      case SUB:
        ...
        break;
      default:
        if (cdu_parameters.NAV_message_counter > 2500) {
 ...
 * Description:
 * 				This function is used to navigate on the NAV
 * Page 2 according to the Keyboards input.
 *
 */
void nav_p2_screen_handler(uint8_t key) {
 ...
 * Description:
 * 				This function is used to navigate on the NAV
 * Page 1 Program screen according to the Keyboards input.
 *
 */
void nav_p1_prog_screen_handler(uint8_t key) {
 ...
 * Description:
 * 				This function is used to navigate on the NAV
 * Page 2 Program screen according to the Keyboards input.
 *
 */
void nav_p2_prog_screen_handler(uint8_t key) {
 ...
 *
 * Description:
 * 				This function is used to navigate on the ADF
 * Page 1 according to the Keyboards input.
 *
 */
void adf_p1_screen_handler(uint8_t key) {
  ...
  if (Input_mode) {
    if (key == BACK) {
      ...
      //			{
      //				bool v_status =
      // input_volume_validation(value); if(v_status)
      //				{
      //					cdu_parameters.volume_adf =
      // value;
      // tft_lcd_send_command_("t3",TEXT, cdu_parameters.volume_adf, NULL);
      // update_flash();
      //					// TODO: add the  Transmission
      // functions
      //				}
      //				else
      //				{
      //					// show popup
      //					tft_lcd_send_command_int("t11",
      // APH, SHOW_POPUP); 					invalid_entry =
      // true;
      //				}
      //			}
      if (strcmp(prefix, STANDBY) == 0) {
      ...
      if ((key >= '0' && key <= '9')) {
        ...
        //				else if(input[0] == 'V')
        //				{
        //					tft_lcd_send_command_text("t3",TEXT,
        // input);
        //				}
      }
    }
    ...
    switch (key) {
      ...
      case ADD:
        if (cdu_parameters.volume_adf < VOLUME_MAX) {
          ...
          update_flash();
          /* Send the volume message */
          send_to_uart_queue_volume(ADF, VOLUME, cdu_parameters.volume_adf);
        }
        break;
      case SUB:
 ...
 *
 * Description:
 * 				This function is used to navigate on the TACAN
 * Page 1 screen according to the Keyboards input.
 *
 */
void tacan_p1_ch_screen_handler(uint8_t key) {
  ...
  if (Input_mode) {
    ...
    switch (key) {
      ...
      case LEFT_SW2:
        break;
      case LEFT_SW3:
        send_to_uart_tacan_mode(TACAN, MODE, MODE_FREQUENCY);
        ...
        //				tft_lcd_send_command_("t3", TEXT,
        // cdu_parameters.volume_tacan, NULL);
        break;
      case LEFT_SW4:
        break;
      case RIGHT_SW1:
      ...
      case ADD:
        if (cdu_parameters.volume_tacan < VOLUME_MAX) {
          ...
          update_flash();
          /* Send the volume message */
          send_to_uart_queue_volume(TACAN, VOLUME, cdu_parameters.volume_tacan);
        }
        break;
      case SUB:
 ...
 * Description:
 * 				This function is used to navigate on the TACAN
 * Page 2 screen according to the Keyboards input.
 *
 */
void tacan_p2_ch_screen_handler(uint8_t key) {
 ...
 * Description:
 * 				This function is used to navigate on the TACAN
 * program Page screen according to the Keyboards input.
 *
 */
void tacan_p1_ch_prog_screen_handler(uint8_t key) {
    ...
    if (key == BACK) {
      ...
      if (status) {
        if (strcmp(prefix, PRE_PRO_1) == 0) {
          ...
          tft_lcd_send_command_channel("t10", TEXT, cdu_parameters.p1_channel, PRE_PRO_1);
          //					update_corresponding_frequency(PRE_PRO_1,
          // cdu_parameters.p1_channel, &cdu_parameters);
        }
        if (strcmp(prefix, PRE_PRO_2) == 0) {
          memset(cdu_parameters.p2_channel, 0, TACAN_S);
          memcpy(cdu_parameters.p2_channel, value, strlen(value));
          tft_lcd_send_command_channel("t10", TEXT, cdu_parameters.p2_channel, PRE_PRO_2);
          //					update_corresponding_frequency(PRE_PRO_2,
          // cdu_parameters.p2_channel, &cdu_parameters);
        }
        if (strcmp(prefix, PRE_PRO_3) == 0) {
          memset(cdu_parameters.p3_channel, 0, TACAN_S);
          memcpy(cdu_parameters.p3_channel, value, strlen(value));
          tft_lcd_send_command_channel("t10", TEXT, cdu_parameters.p3_channel, PRE_PRO_3);
          //					update_corresponding_frequency(PRE_PRO_3,
          // cdu_parameters.p3_channel, &cdu_parameters);
        }
        if (strcmp(prefix, PRE_PRO_4) == 0) {
          memset(cdu_parameters.p4_channel, 0, TACAN_S);
          memcpy(cdu_parameters.p4_channel, value, strlen(value));
          tft_lcd_send_command_channel("t10", TEXT, cdu_parameters.p4_channel, PRE_PRO_4);
          //					update_corresponding_frequency(PRE_PRO_4,
          // cdu_parameters.p4_channel, &cdu_parameters);
        }
        update_flash();
      } else {
        // show pop up
    ...
    if (key == BACK) {
      ...
      if (status) {
        if (strcmp(prefix, PRE_PRO_5) == 0) {
          ...
          tft_lcd_send_command_channel("t10", TEXT, cdu_parameters.p5_channel, PRE_PRO_5);
          //					update_corresponding_frequency(PRE_PRO_5,
          // cdu_parameters.p5_channel, &cdu_parameters);
        }
        if (strcmp(prefix, PRE_PRO_6) == 0) {
          memset(cdu_parameters.p6_channel, 0, TACAN_S);
          memcpy(cdu_parameters.p6_channel, value, strlen(value));
          tft_lcd_send_command_channel("t10", TEXT, cdu_parameters.p6_channel, PRE_PRO_6);
          //					update_corresponding_frequency(PRE_PRO_6,
          // cdu_parameters.p6_channel, &cdu_parameters);
        }
        if (strcmp(prefix, PRE_PRO_7) == 0) {
          memset(cdu_parameters.p7_channel, 0, TACAN_S);
          memcpy(cdu_parameters.p7_channel, value, strlen(value));
          tft_lcd_send_command_channel("t10", TEXT, cdu_parameters.p7_channel, PRE_PRO_7);
          //					update_corresponding_frequency(PRE_PRO_7,
          // cdu_parameters.p7_channel, &cdu_parameters);
        }
        if (strcmp(prefix, PRE_PRO_8) == 0) {
          memset(cdu_parameters.p8_channel, 0, TACAN_S);
          memcpy(cdu_parameters.p8_channel, value, strlen(value));
          tft_lcd_send_command_channel("t10", TEXT, cdu_parameters.p8_channel, PRE_PRO_8);
          //					update_corresponding_frequency(PRE_PRO_8,
          // cdu_parameters.p8_channel, &cdu_parameters);
        }
        update_flash();
      } else {
        // show pop up
 ...
 * Description:
 * 				This function is used to navigate on the TACAN
 * Frequency Page 1 according to the Keyboards input.
 *
 */
void tacan_p1_fq_screen_handler(uint8_t key) {
  ...
  //	{
  //		tft_lcd_send_command_int("t11", APH, HIDE_POPUP);
  //		tft_lcd_send_command_float("t1",TEXT,
  // cdu_parameters.standby_tacan_freq, STANDBY);
  //		tft_lcd_send_command_float("t4", TEXT,
  // cdu_parameters.active_tacan_freq, ACTIVE);
  // tft_lcd_send_command_("t3", TEXT, cdu_parameters.volume_tacan, NULL);
  // invalid_entry = false; 		return;
  //	}
  //
  //	if(Input_mode)
  //	{
  ...
  //			{
  //				if(strcmp(prefix,STANDBY) == 0) {
  //					cdu_parameters.standby_tacan_freq =
  // value; tft_lcd_send_command_float("t1",TEXT,
  // cdu_parameters.standby_tacan_freq, STANDBY);
  //					update_corresponding_channel(STANDBY,
  // cdu_parameters.standby_tacan_freq, &cdu_parameters);
  // update_flash();
  // send_to_uart_queue_freq(TACAN, A_FREQ, cdu_parameters.active_tacan_freq);
  // send_to_uart_queue_freq(TACAN, S_FREQ, cdu_parameters.standby_tacan_freq);
  //				}
  ...
  //			{
  //				// show popup
  //				tft_lcd_send_command_int("t11", APH,
  // SHOW_POPUP); 				invalid_entry = true;
  //			}
  //			Input_mode = false;
  //			stop_timer_cursor();
  //		}
  ...
  switch (key) {
    case LEFT_SW1:
      ...
      break;
    case LEFT_SW2:
      send_to_uart_tacan_mode(TACAN, MODE, MODE_CHANNEL);
      tft_lcd_change_screen(TACAN_P1_CH_SCREEN);
      tft_lcd_send_command_channel("t1", TEXT, cdu_parameters.standby_channel, STANDBY);
      tft_lcd_send_command_channel("t4", TEXT, cdu_parameters.active_channel, ACTIVE);
      tft_lcd_send_command_("t3", TEXT, cdu_parameters.volume_tacan, NULL);
    ...
    case RIGHT_SW1:
      ...
      //				tft_lcd_send_command_float("t4", TEXT,
      // cdu_parameters.active_tacan_freq, ACTIVE);
      //				tft_lcd_send_command_float("t10", TEXT,
      // cdu_parameters.p1_tacan_freq, PRE_PRO_1);
      // tft_lcd_send_command_int("t5", COLOR_CHG_BG, BLACK);
      // tft_lcd_send_command_int("t5", COLOR_CHG_TEXT, WHITE);
      // prefix_index = '1';
      break;
    case RIGHT_SW2:
      ...
      // cdu_parameters.active_tacan_freq, ACTIVE);
      //				tft_lcd_send_command_float("t10", TEXT,
      // cdu_parameters.p2_tacan_freq, PRE_PRO_2);
      // tft_lcd_send_command_int("t6", COLOR_CHG_BG, BLACK);
      // tft_lcd_send_command_int("t6", COLOR_CHG_TEXT, WHITE);
      // prefix_index = '2';
      break;
    case RIGHT_SW3:
      ...
      // cdu_parameters.active_tacan_freq, ACTIVE);
      //				tft_lcd_send_command_float("t10", TEXT,
      // cdu_parameters.p3_tacan_freq, PRE_PRO_3);
      // tft_lcd_send_command_int("t7", COLOR_CHG_BG, BLACK);
      // tft_lcd_send_command_int("t7", COLOR_CHG_TEXT, WHITE);
      // prefix_index = '3';
      break;
    case RIGHT_SW4:
      ...
      // cdu_parameters.active_tacan_freq, ACTIVE);
      //				tft_lcd_send_command_float("t10", TEXT,
      // cdu_parameters.p4_tacan_freq, PRE_PRO_4);
      // tft_lcd_send_command_int("t8", COLOR_CHG_BG, BLACK);
      // tft_lcd_send_command_int("t8", COLOR_CHG_TEXT, WHITE);
      // prefix_index = '4';
      break;
    ...
    case SWAP:
      ...
      //				tft_lcd_send_command_float("t4", TEXT,
      // cdu_parameters.active_tacan_freq, ACTIVE);
      //				update_corresponding_channel(ACTIVE,
      // cdu_parameters.active_tacan_freq, &cdu_parameters);
      // update_flash(); 				send_to_uart_queue_freq(TACAN,
      // A_FREQ, cdu_parameters.active_tacan_freq); send_to_uart_queue_freq(TACAN,
      // S_FREQ, cdu_parameters.standby_tacan_freq);
      break;
    case ADD:
      //				if (cdu_parameters.volume_tacan < VOLUME_MAX)
      //				{
      ...
      // update_flash();
      //					/* Send the volume message */
      //					send_to_uart_queue_volume(TACAN, VOLUME,
      // cdu_parameters.volume_tacan);
      //				}
      break;
    case SUB:
 ...
 * Description:
 * 				This function is used to navigate on the TACAN
 * Frequency Page 2 according to the Keyboards input.
 *
 */
void tacan_p2_fq_screen_handler(uint8_t key) {
  ...
  switch (key) {
    ...
    case RIGHT_SW1:
      ...
      // cdu_parameters.active_tacan_freq, ACTIVE);
      //				tft_lcd_send_command_float("t10", TEXT,
      // cdu_parameters.p5_tacan_freq, PRE_PRO_5);
      // tft_lcd_send_command_int("t5", COLOR_CHG_BG, BLACK);
      // tft_lcd_send_command_int("t5", COLOR_CHG_TEXT, WHITE);
      // prefix_index = '5';
      break;
    case RIGHT_SW2:
      ...
      // cdu_parameters.active_tacan_freq, ACTIVE);
      //				tft_lcd_send_command_float("t10", TEXT,
      // cdu_parameters.p6_tacan_freq, PRE_PRO_6);
      // tft_lcd_send_command_int("t6", COLOR_CHG_BG, BLACK);
      // tft_lcd_send_command_int("t6", COLOR_CHG_TEXT, WHITE);
      // prefix_index = '6';
      break;
    case RIGHT_SW3:
      ...
      // cdu_parameters.active_tacan_freq, ACTIVE);
      //				tft_lcd_send_command_float("t10", TEXT,
      // cdu_parameters.p7_tacan_freq, PRE_PRO_7);
      // tft_lcd_send_command_int("t7", COLOR_CHG_BG, BLACK);
      // tft_lcd_send_command_int("t7", COLOR_CHG_TEXT, WHITE);
      // prefix_index = '7';
      break;
    case RIGHT_SW4:
      ...
      // cdu_parameters.active_tacan_freq, ACTIVE);
      //				tft_lcd_send_command_float("t10", TEXT,
      // cdu_parameters.p8_tacan_freq, PRE_PRO_8);
      // tft_lcd_send_command_int("t8", COLOR_CHG_BG, BLACK);
      // tft_lcd_send_command_int("t8", COLOR_CHG_TEXT, WHITE);
      // prefix_index = '8';
      break;
 ...
 * Description:
 * 				This function is used to navigate on the TACAN
 * Frequency Page 1 Program screen according to the Keyboards input.
 *
 */
void tacan_p1_fq_prog_screen_handler(uint8_t key) {
  ...
  //			case '1':
  //				tft_lcd_send_command_float("t10", TEXT,
  // cdu_parameters.p1_tacan_freq, PRE_PRO_1); 				break;
  // case '2': 				tft_lcd_send_command_float("t10", TEXT,
  // cdu_parameters.p2_tacan_freq, PRE_PRO_2); 				break;
  // case '3': 				tft_lcd_send_command_float("t10", TEXT,
  // cdu_parameters.p3_tacan_freq, PRE_PRO_3); 				break;
  // case '4': 				tft_lcd_send_command_float("t10", TEXT,
  // cdu_parameters.p4_tacan_freq, PRE_PRO_4); 				break;
  //		}
  //		invalid_entry = false;
  //		return;
  //	}
  ...
  //					tft_lcd_send_command_float("t10",TEXT,
  // cdu_parameters.p1_tacan_freq, PRE_PRO_1);
  //					update_corresponding_channel(PRE_PRO_1,
  // cdu_parameters.p1_tacan_freq, &cdu_parameters);
  //				}
  //				if(strcmp(prefix,PRE_PRO_2) == 0) {
  //					cdu_parameters.p2_tacan_freq = value;
  //					tft_lcd_send_command_float("t10",TEXT,
  // cdu_parameters.p2_tacan_freq, PRE_PRO_2);
  //					update_corresponding_channel(PRE_PRO_2,
  // cdu_parameters.p2_tacan_freq, &cdu_parameters);
  //
  //				}
  //				if(strcmp(prefix,PRE_PRO_3) == 0) {
  ...
  //					tft_lcd_send_command_float("t10",TEXT,
  // cdu_parameters.p3_tacan_freq, PRE_PRO_3);
  //					update_corresponding_channel(PRE_PRO_3,
  // cdu_parameters.p3_tacan_freq, &cdu_parameters);
  //
  //				}
  //				if(strcmp(prefix,PRE_PRO_4) == 0) {
  ...
  //					tft_lcd_send_command_float("t10",TEXT,
  // cdu_parameters.p4_tacan_freq, PRE_PRO_4);
  //					update_corresponding_channel(PRE_PRO_4,
  // cdu_parameters.p4_tacan_freq, &cdu_parameters);
  //
  //				}
  //				update_flash();
  //			}
  ...
  //				tft_lcd_send_command_float("t4", TEXT,
  // cdu_parameters.active_tacan_freq, ACTIVE);
  // tft_lcd_send_command_float("t10", TEXT, cdu_parameters.p1_tacan_freq,
  // PRE_PRO_1); 				tft_lcd_send_command_int("t5",
  // COLOR_CHG_BG, BLACK); tft_lcd_send_command_int("t5", COLOR_CHG_TEXT,
  // WHITE); prefix_index = '1'; 				break;
  // case RIGHT_SW2:
  // tft_lcd_change_screen(TACAN_P1_FQ_PROG_SCREEN);
  //				tft_lcd_send_command_float("t1",TEXT,
  // cdu_parameters.standby_tacan_freq, STANDBY);
  //				tft_lcd_send_command_float("t4", TEXT,
  // cdu_parameters.active_tacan_freq, ACTIVE);
  // tft_lcd_send_command_float("t10", TEXT, cdu_parameters.p2_tacan_freq,
  // PRE_PRO_2); 				tft_lcd_send_command_int("t6",
  // COLOR_CHG_BG, BLACK); tft_lcd_send_command_int("t6", COLOR_CHG_TEXT,
  // WHITE); prefix_index = '2'; 				break;
  // case RIGHT_SW3:
  // tft_lcd_change_screen(TACAN_P1_FQ_PROG_SCREEN);
  //				tft_lcd_send_command_float("t1",TEXT,
  // cdu_parameters.standby_tacan_freq, STANDBY);
  //				tft_lcd_send_command_float("t4", TEXT,
  // cdu_parameters.active_tacan_freq, ACTIVE);
  // tft_lcd_send_command_float("t10", TEXT, cdu_parameters.p3_tacan_freq,
  // PRE_PRO_3); 				tft_lcd_send_command_int("t7",
  // COLOR_CHG_BG, BLACK); tft_lcd_send_command_int("t7", COLOR_CHG_TEXT,
  // WHITE); prefix_index = '3'; 				break;
  // case RIGHT_SW4:
  // tft_lcd_change_screen(TACAN_P1_FQ_PROG_SCREEN);
  //				tft_lcd_send_command_float("t1",TEXT,
  // cdu_parameters.standby_tacan_freq, STANDBY);
  //				tft_lcd_send_command_float("t4", TEXT,
  // cdu_parameters.active_tacan_freq, ACTIVE);
  // tft_lcd_send_command_float("t10", TEXT, cdu_parameters.p4_tacan_freq,
  // PRE_PRO_4); 				tft_lcd_send_command_int("t8",
  // COLOR_CHG_BG, BLACK); tft_lcd_send_command_int("t8", COLOR_CHG_TEXT,
  // WHITE); prefix_index = '4'; 				break;
  // case HOME: 				tft_lcd_Home(1);
  // prefix_index = 0; 				break; 			case
  // DIM: 				break; 			case BRT:
  // break; 			case BACK:
  //				tft_lcd_change_screen(TACAN_P1_FQ_SCREEN);
  //				tft_lcd_send_command_float("t1",TEXT,
  ...
  // tft_lcd_send_command_("t3", TEXT, cdu_parameters.volume_tacan, NULL);
  // break; 			case OK:
  //				copy_preset_to_standby_freq(prefix_index,
  // TACAN); 				update_flash();
  // update_corresponding_channel(STANDBY, cdu_parameters.standby_tacan_freq,
  // &cdu_parameters); 				send_to_uart_queue_freq(TACAN,
  // A_FREQ, cdu_parameters.active_tacan_freq); send_to_uart_queue_freq(TACAN,
  // S_FREQ, cdu_parameters.standby_tacan_freq);
  // tft_lcd_send_command_float("t1", TEXT, cdu_parameters.standby_tacan_freq,
  // STANDBY); 				switch(prefix_index)
  //				{
  //					case '1':
  //						tft_lcd_send_command_float("t10",
  // TEXT, cdu_parameters.p1_tacan_freq, PRE_PRO_1);
  // break; 					case '2':
  //						tft_lcd_send_command_float("t10",
  // TEXT, cdu_parameters.p2_tacan_freq, PRE_PRO_2);
  // break; 					case '3':
  //						tft_lcd_send_command_float("t10",
  // TEXT, cdu_parameters.p3_tacan_freq, PRE_PRO_3);
  // break; 					case '4':
  //						tft_lcd_send_command_float("t10",
  // TEXT, cdu_parameters.p4_tacan_freq, PRE_PRO_4);
  // break;
  //				}
  //				prefix_index = 0;
  //				vTaskDelay(50);
 ...
 * Description:
 * 				This function is used to navigate on the TACAN
 * Frequency Page 2 Program screen according to the Keyboards input.
 *
 */
void tacan_p2_fq_prog_screen_handler(uint8_t key) {
  ...
  //			case '5':
  //				tft_lcd_send_command_float("t10", TEXT,
  // cdu_parameters.p5_tacan_freq, PRE_PRO_5); 				break;
  // case '6': 				tft_lcd_send_command_float("t10", TEXT,
  // cdu_parameters.p6_tacan_freq, PRE_PRO_6); 				break;
  // case '7': 				tft_lcd_send_command_float("t10", TEXT,
  // cdu_parameters.p7_tacan_freq, PRE_PRO_7); 				break;
  // case '8': 				tft_lcd_send_command_float("t10", TEXT,
  // cdu_parameters.p8_tacan_freq, PRE_PRO_8); 				break;
  //		}
  //		invalid_entry = false;
  //		return;
  //	}
  ...
  //					tft_lcd_send_command_float("t10",TEXT,
  // cdu_parameters.p5_tacan_freq, PRE_PRO_5);
  //					update_corresponding_channel(PRE_PRO_5,
  // cdu_parameters.p5_tacan_freq, &cdu_parameters);
  //
  //				}
  //				if(strcmp(prefix,PRE_PRO_6) == 0) {
  ...
  //					tft_lcd_send_command_float("t10",TEXT,
  // cdu_parameters.p6_tacan_freq, PRE_PRO_6);
  //					update_corresponding_channel(PRE_PRO_6,
  // cdu_parameters.p6_tacan_freq, &cdu_parameters);
  //
  //				}
  //				if(strcmp(prefix,PRE_PRO_7) == 0) {
  ...
  //					tft_lcd_send_command_float("t10",TEXT,
  // cdu_parameters.p7_tacan_freq, PRE_PRO_7);
  //					update_corresponding_channel(PRE_PRO_7,
  // cdu_parameters.p7_tacan_freq, &cdu_parameters);
  //
  //				}
  //				if(strcmp(prefix,PRE_PRO_8) == 0) {
  ...
  //					tft_lcd_send_command_float("t10",TEXT,
  // cdu_parameters.p8_tacan_freq, PRE_PRO_8);
  //					update_corresponding_channel(PRE_PRO_8,
  // cdu_parameters.p8_tacan_freq, &cdu_parameters);
  //
  //				}
  //				update_flash();
  //			}
  ...
  //				tft_lcd_send_command_float("t4", TEXT,
  // cdu_parameters.active_tacan_freq, ACTIVE);
  // tft_lcd_send_command_float("t10", TEXT, cdu_parameters.p5_tacan_freq,
  // PRE_PRO_5); 				tft_lcd_send_command_int("t5",
  // COLOR_CHG_BG, BLACK); tft_lcd_send_command_int("t5", COLOR_CHG_TEXT,
  // WHITE); prefix_index = '5'; 				break;
  // case RIGHT_SW2:
  // tft_lcd_change_screen(TACAN_P2_FQ_PROG_SCREEN);
  //				tft_lcd_send_command_float("t1",TEXT,
  // cdu_parameters.standby_tacan_freq, STANDBY);
  //				tft_lcd_send_command_float("t4", TEXT,
  // cdu_parameters.active_tacan_freq, ACTIVE);
  // tft_lcd_send_command_float("t10", TEXT, cdu_parameters.p6_tacan_freq,
  // PRE_PRO_6); 				tft_lcd_send_command_int("t6",
  // COLOR_CHG_BG, BLACK); tft_lcd_send_command_int("t6", COLOR_CHG_TEXT,
  // WHITE); prefix_index = '6'; 				break;
  // case RIGHT_SW3:
  // tft_lcd_change_screen(TACAN_P2_FQ_PROG_SCREEN);
  //				tft_lcd_send_command_float("t1",TEXT,
  // cdu_parameters.standby_tacan_freq, STANDBY);
  //				tft_lcd_send_command_float("t4", TEXT,
  // cdu_parameters.active_tacan_freq, ACTIVE);
  // tft_lcd_send_command_float("t10", TEXT, cdu_parameters.p7_tacan_freq,
  // PRE_PRO_7); 				tft_lcd_send_command_int("t7",
  // COLOR_CHG_BG, BLACK); tft_lcd_send_command_int("t7", COLOR_CHG_TEXT,
  // WHITE); prefix_index = '7'; 				break;
  // case RIGHT_SW4:
  // tft_lcd_change_screen(TACAN_P2_FQ_PROG_SCREEN);
  //				tft_lcd_send_command_float("t1",TEXT,
  // cdu_parameters.standby_tacan_freq, STANDBY);
  //				tft_lcd_send_command_float("t4", TEXT,
  // cdu_parameters.active_tacan_freq, ACTIVE);
  // tft_lcd_send_command_float("t10", TEXT, cdu_parameters.p8_tacan_freq,
  // PRE_PRO_8); 				tft_lcd_send_command_int("t8",
  // COLOR_CHG_BG, BLACK); tft_lcd_send_command_int("t8", COLOR_CHG_TEXT,
  // WHITE); prefix_index = '8'; 				break;
  // case HOME: 				tft_lcd_Home(1);
  // break; 			case DIM: 				break;
  // case BRT: 				break; 			case BACK:
  // tft_lcd_change_screen(TACAN_P2_FQ_SCREEN);
  //				tft_lcd_send_command_float("t1",TEXT,
  // cdu_parameters.standby_tacan_freq, STANDBY);
  //				tft_lcd_send_command_float("t4", TEXT,
  ...
  //					case '5':
  //						tft_lcd_send_command_float("t10",
  // TEXT, cdu_parameters.p5_tacan_freq, PRE_PRO_5);
  // break; 					case '6':
  //						tft_lcd_send_command_float("t10",
  // TEXT, cdu_parameters.p6_tacan_freq, PRE_PRO_6);
  // break; 					case '7':
  //						tft_lcd_send_command_float("t10",
  // TEXT, cdu_parameters.p7_tacan_freq, PRE_PRO_7);
  // break; 					case '8':
  //						tft_lcd_send_command_float("t10",
  // TEXT, cdu_parameters.p8_tacan_freq, PRE_PRO_8);
  // break;
  //				}
  //				prefix_index = 0;
  //				vTaskDelay(50);
 ...
 *
 * Description:
 * 				This function is used to navigate on the HF Page
 * 1 screen according to the Keyboards input.
 *
 */
void hf_p1_screen_handler(uint8_t key) {
  ...
  if (Input_mode) {
    ...
    switch (key) {
      case LEFT_SW1:
        ...
        break;
      case LEFT_SW2:
        tft_lcd_send_command_int("t2", COLOR_CHG_BG, WHITE);
        break;
      case LEFT_SW3:
        break;
      case LEFT_SW4:
 ...
 * Description:
 * 				This function is used to navigate on the HF Page
 * 2 screen according to the Keyboards input.
 *
 */
void hf_p2_screen_handler(uint8_t key) {
  ...
  if (Input_mode) {
    ...
    switch (key) {
      ...
      case NEXT:
        tft_lcd_change_screen(HF_P3_SCREEN);
        tft_lcd_send_command_float("t1", TEXT, cdu_parameters.standby_hf_freq, STANDBY);
        tft_lcd_send_command_float("t4", TEXT, cdu_parameters.active_hf_freq, ACTIVE);
        break;
      case DIM:
...
void hf_p3_screen_handler(uint8_t key) {
  ...
  if (Input_mode) {
    ...
    switch (key) {
      ...
      case NEXT:
        tft_lcd_change_screen(HF_P4_SCREEN);
        tft_lcd_send_command_float("t1", TEXT, cdu_parameters.standby_hf_freq, STANDBY);
        tft_lcd_send_command_float("t4", TEXT, cdu_parameters.active_hf_freq, ACTIVE);
        break;
...
void hf_p4_screen_handler(uint8_t key) {  // current_screen = HF_P2_SCREEN;
  if (invalid_entry) {
    tft_lcd_send_command_int("t11", APH, HIDE_POPUP);
    tft_lcd_send_command_float("t1", TEXT, cdu_parameters.standby_hf_freq, STANDBY);
    tft_lcd_send_command_float("t4", TEXT, cdu_parameters.active_hf_freq, ACTIVE);
  ...
  if (Input_mode) {
    ...
    switch (key) {
      ...
      case NEXT:
        tft_lcd_change_screen(HF_P5_SCREEN);
        tft_lcd_send_command_float("t1", TEXT, cdu_parameters.standby_hf_freq, STANDBY);
        tft_lcd_send_command_float("t4", TEXT, cdu_parameters.active_hf_freq, ACTIVE);
        break;
      case BACK:
...
void hf_p5_screen_handler(uint8_t key) {
  ...
  if (Input_mode) {
    ...
    switch (key) {
      ...
      case NEXT:
        tft_lcd_change_screen(HF_P6_SCREEN);
        tft_lcd_send_command_float("t1", TEXT, cdu_parameters.standby_hf_freq, STANDBY);
        tft_lcd_send_command_float("t4", TEXT, cdu_parameters.active_hf_freq, ACTIVE);
        break;
      case BACK:
 ...
 * Description:
 * 				This function is used to navigate on the HF Page
 * 1 Program screen according to the Keyboards input.
 *
 */
void hf_p1_prog_screen_handler(uint8_t key) {
 ...
 * Description:
 * 				This function is used to navigate on the HF Page
 * 2 Program screen according to the Keyboards input.
 *
 */
void hf_p2_prog_screen_handler(uint8_t key) {
 ...
 *
 * Description:
 * 				This function is used to navigate on the VHF
 * Page 1 screen according to the Keyboards input.
 *
 */
void vhf_p1_screen_handler(uint8_t key) {
 ...
 *
 * Description:
 * 				This function is used to navigate on the UHF
 * Page 1 screen according to the Keyboards input.
 *
 */
void uhf_p1_screen_handler(uint8_t key) {
 ...
 * correct screen handler and pass the keyboards input.
 *
 */
void tft_lcd(uint8_t key) {
  if (key != NDF) {
...
void update_screen_(TFT_Screen screen) {
  ...
  switch (cls) {
    ...
    case TACAN:
      ...
      Message_Id m_id = get_message_Id();
      if (m_id == S_FREQ) {
      ...
      break;
    case HF:
      if ((screen == HF_P1_SCREEN) || (screen == HF_P2_SCREEN) || (screen == HF_P1_PROG_SCREEN) ||
          (screen == HF_P2_PROG_SCREEN)) {
        tft_lcd_send_command_float("t1", TEXT, cdu_parameters.standby_hf_freq, STANDBY);
        tft_lcd_send_command_float("t4", TEXT, cdu_parameters.active_hf_freq, ACTIVE);
        //				update_flash();
      }
      break;
    case VHF:
      if ((screen == VHF_P1_SCREEN) || (screen == VHF_P2_SCREEN) ||
          (screen == VHF_P1_PROG_SCREEN) || (screen == VHF_P2_PROG_SCREEN)) {
        tft_lcd_send_command_float("t1", TEXT, cdu_parameters.standby_vhf_freq, STANDBY);
        tft_lcd_send_command_float("t4", TEXT, cdu_parameters.active_vhf_freq, ACTIVE);
        //				update_flash();
      }
      break;
    case UHF:
      if ((screen == UHF_P1_SCREEN) || (screen == UHF_P2_SCREEN) ||
          (screen == UHF_P1_PROG_SCREEN) || (screen == UHF_P2_PROG_SCREEN)) {
        tft_lcd_send_command_float("t1", TEXT, cdu_parameters.standby_uhf_freq, STANDBY);
        tft_lcd_send_command_float("t4", TEXT, cdu_parameters.active_uhf_freq, ACTIVE);
        update_flash();
      }
