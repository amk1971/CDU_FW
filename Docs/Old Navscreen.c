...
//	bool decimal_added;
	...
	case Idle:
		...
		else if(softkey == R4)
		{
			...
			if(!(Params->page)){
				ptrFreq = &Params->P4.freq;
				strncpy(Format, "P4 %0.3f", 10);
			}else {
				ptrFreq = &Params->P8.freq;
	...
	case RightSoftKey:
		...
		if ( (HAL_GetTick() - Preset_Timer) > 15000) { // Wait 15 second and return if no action
			NavScreenState = idle;
