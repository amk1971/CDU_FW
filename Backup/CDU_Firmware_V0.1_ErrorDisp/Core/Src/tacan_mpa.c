...
ChannelFrequency tacan_freq_channel_map[TACAN_FREQ_CHANNEL_MAP_SIZE] = {
    {"1X",134.40},
    {"1Y",134.45},
    {"2X",134.50},
    {"2Y",134.55},
...
int8_t custom_compare(const char *a, const char *b)
{
    ...
    while (isdigit((unsigned char)*alpha_a)) alpha_a++;
    while (isdigit((unsigned char)*alpha_b)) alpha_b++;
...
int16_t search_tacan_channel_(const char *key, uint16_t low, uint16_t high)
{
    ...
    {
        mid = low + (high - low) / 2;
        int8_t cmp_result = custom_compare(tacan_freq_channel_map[mid].channel, key);   //strcmp(key, tacan_freq_channel_map[mid].channel);
        if( cmp_result == 0)
        {
            return mid;
        }
...
{
    uint16_t len = sizeof(tacan_freq_channel_map)/sizeof(tacan_freq_channel_map[0]);
    int16_t index = 0;
...
int8_t find_channel(float freq, char *key)
{
    ...
    int16_t index = 0;
    uint8_t size;
    index = search_tacan_frequency_(freq, len -1);
    if(index == -1)
    {
    ...
    {
        size = strlen(tacan_freq_channel_map[index].channel);  // Get the length of the channel string
        strncpy(key, tacan_freq_channel_map[index].channel, size);  // Copy the channel to the key buffer
        key[size+1] = '\0';
        return 0;  // Key found in the array
    }
    return -1; // Key not found in the
}
 ...
 */
void update_corresponding_frequency(const char * identifier, const char *key, CDU_Parameters *param)
{
	float f_val = 0;
	...
	if(f_val >= 0)
	{
		if(strcmp(identifier, STANDBY))
		{
			param->standby_tacan_freq = f_val;
		}
		else if(strcmp(identifier, ACTIVE))
		{
			param->active_tacan_freq = f_val;
		}
		else if(strcmp(identifier, PRE_PRO_1))
		{
			param->p1_tacan_freq = f_val;
		}
		else if(strcmp(identifier, PRE_PRO_2))
		{
			param->p2_tacan_freq = f_val;
		}
		else if(strcmp(identifier, PRE_PRO_3))
		{
			param->p3_tacan_freq = f_val;
		}
		else if(strcmp(identifier, PRE_PRO_4))
		{
			param->p4_tacan_freq = f_val;
		}
		else if(strcmp(identifier, PRE_PRO_5))
		{
			param->p5_tacan_freq = f_val;
		}
		else if(strcmp(identifier, PRE_PRO_6))
		{
			param->p6_tacan_freq = f_val;
		}
		else if(strcmp(identifier, PRE_PRO_7))
		{
			param->p7_tacan_freq = f_val;
		}
		else if(strcmp(identifier, PRE_PRO_8))
		{
 ...
 * - param: Pointer to the CDU_Parameters structure to be updated.
 */
void update_corresponding_channel(const char *identifier, float f_val, CDU_Parameters *param)
{
	...
	if(status >= 0)
	{
		if(strcmp(identifier, STANDBY))
		{
			memcpy(param->standby_channel, tacan_channel, strlen(tacan_channel));
		}
		else if(strcmp(identifier, ACTIVE))
		{
			memcpy(param->active_channel, tacan_channel, strlen(tacan_channel));
		}
		else if(strcmp(identifier, PRE_PRO_1))
		{
			memcpy(param->p1_channel, tacan_channel, strlen(tacan_channel));
		}
		else if(strcmp(identifier, PRE_PRO_2))
		{
			memcpy(param->p2_channel, tacan_channel, strlen(tacan_channel));
		}
		else if(strcmp(identifier, PRE_PRO_3))
		{
			memcpy(param->p3_channel, tacan_channel, strlen(tacan_channel));
		}
		else if(strcmp(identifier, PRE_PRO_4))
		{
			memcpy(param->p4_channel, tacan_channel, strlen(tacan_channel));
		}
		else if(strcmp(identifier, PRE_PRO_5))
		{
			memcpy(param->p5_channel, tacan_channel, strlen(tacan_channel));
		}
		else if(strcmp(identifier, PRE_PRO_6))
		{
			memcpy(param->p6_channel, tacan_channel, strlen(tacan_channel));
		}
		else if(strcmp(identifier, PRE_PRO_7))
		{
			memcpy(param->p7_channel, tacan_channel, strlen(tacan_channel));
		}
		else if(strcmp(identifier, PRE_PRO_8))
		{
			memcpy(param->p8_channel, tacan_channel, strlen(tacan_channel));
		}
