...
uint16_t ROWS_PINS[NO_ROWS_COLS] = { ROW_0_PIN, ROW_1_PIN, ROW_2_PIN, ROW_3_PIN,
		ROW_4_PIN, ROW_5_PIN, ROW_6_PIN, ROW_7_PIN };
uint16_t COLS_PINS[NO_ROWS_COLS] = { COL_0_PIN, COL_1_PIN, COL_2_PIN, COL_3_PIN,
...
GPIO_TypeDef *ROW_PORTS[NO_ROWS_COLS] = { ROW_0_PORT, ROW_1_PORT, ROW_2_PORT,
		ROW_3_PORT, ROW_4_PORT, ROW_5_PORT, ROW_6_PORT, ROW_7_PORT };
GPIO_TypeDef *COL_PORTS[NO_ROWS_COLS] = { COL_0_PORT, COL_1_PORT, COL_2_PORT,
		COL_3_PORT, COL_4_PORT, COL_5_PORT, COL_6_PORT, COL_7_PORT };

const char keyMap[NO_ROWS_COLS][NO_ROWS_COLS] = { { BACK, HOME, OK, SWAP, PREV,
		NEXT, DIM, BRT }, { '1', '2', '3', 'A', 'B', 'C', 'D', 'E' }, // 'F',
		{ 'F', '4', '5', '6', 'G', 'H', 'I', 'J' }, //,
		{ 'K', 'L', '7', '8', '9', 'M', 'N', 'O' }, //,
		{ 'P', 'Q', 'R', 'p', '0', '/', 'S', 'T' }, //,
		{ 'U', 'V', 'W', 'X', INC, NDF, NDF, NDF }, { NDF, 'Y', 'Z', ADD, DEC,
				ENT, SPACE, SPACE }, { CLR, NDF, SUB, NDF, NDF, NDF, NDF, NDF },
...
void keyboard_thread(void *pvParameters) {
	...
	for (;;) {
		...
		if (key != NDF) {
			...
			if (xKeyQueue != NULL) {
				xQueueSend(xKeyQueue, &key, pdMS_TO_TICKS(500));
			}
...
char keyboard_scan(void) {
	for (int row = 0; row < 8; row++) {
		...
		for (int col = 0; col < 8; col++) {
			...
			if (HAL_GPIO_ReadPin(COL_PORTS[col], COLS_PINS[col])
					== GPIO_PIN_RESET) {
...
//		debug_print("I_ Key Pressed RIGHT_SW2 :: %c", panel_key);
		...
		panel_key = RIGHT_SW3;
		Can_TX_SF(Request,0,1,0,1,99,290,725,955);
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
	...
	TickType_t currentTime = xTaskGetTickCountFromISR(); // Use ISR-safe function
	...
	// De-bounce logic (Ensuring safe time check in ISR)
	if (panel_key != NDF
			&& (currentTime - lastPressTime) * portTICK_PERIOD_MS
					>= DEBOUNCE_DELAY_MS) {
		lastPressTime = currentTime;  // Update the last press time

		// Send to queue from ISR
		if (xKeyQueue != NULL) {
			xQueueSendFromISR(xKeyQueue, &panel_key, &xHigherPriorityTaskWoken);
		}
	}
