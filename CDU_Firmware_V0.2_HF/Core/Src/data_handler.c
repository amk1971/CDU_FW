...
#define UPLOAD_DEFAULT 		0	// 0 DIsable and 1 for Enable
...
const char class_id[NUM_CLASSES_ID] = { 'V',	// for VHF NAV
		'A',	// ADF
...
const char *message_id_nav_vhf[NUM_MESSAGE_ID] = { "28", "27", "73", "99",	// health
		};

const char *message_id_adf_tacan[NUM_MESSAGE_ID] = { "29",// Standby Frequency Message ID
		"42",	// Active Frequency Message ID
		"73",	// Volume, Toggle ADF/ANT, Switch on BFO Message ID
		"71",
...
void notify_tft_tacan(tacan_mode mode);
Class_Id get_class_Id(void);
float get_freq(uint8_t f_mhz, uint16_t f_khz);
void handle_NAV_incoming_message(Identifier *ident, uint8_t f_mhz,
		uint16_t f_khz, uint8_t vol);
void handle_ADF_incoming_message(Identifier *ident, uint8_t f_mhz,
		uint16_t f_khz, uint8_t vol);
void handle_TACAN_incoming_message(Identifier *ident, uint8_t f_mhz,
		uint16_t f_khz, uint8_t vol);
void handle_HF_incoming_message(Identifier *ident, uint8_t f_mhz,
		uint16_t f_khz, uint8_t vol);
void handle_VHF_incoming_message(Identifier *ident, uint8_t f_mhz,
		uint16_t f_khz, uint8_t vol);
void handle_UHF_incoming_message(Identifier *ident, uint8_t f_mhz,
		uint16_t f_khz, uint8_t vol);
...
MessageHandler message_parse[class_ID_Count] = { handle_NAV_incoming_message,
		handle_ADF_incoming_message, handle_TACAN_incoming_message,
		handle_HF_incoming_message, handle_VHF_incoming_message,
...
void swap_active_standby_channel(void) {
	...
	strncpy(cdu_parameters.active_channel, cdu_parameters.standby_channel,
			sizeof(cdu_parameters.active_channel) - 1);
	cdu_parameters.active_channel[sizeof(cdu_parameters.active_channel) - 1] =
			'\0';  // Ensure active_channel is null-terminated

	// Copy temp (previous active_channel) to standby_channel
	memset(cdu_parameters.standby_channel, 0, TACAN_S);
	strncpy(cdu_parameters.standby_channel, temp,
			sizeof(cdu_parameters.standby_channel) - 1);
	cdu_parameters.standby_channel[sizeof(cdu_parameters.standby_channel) - 1] =
			'\0';  // Ensure standby_channel is null-terminated
...
void copy_preset_to_standby_channel(char index) {
	...
	case '1':
		memset(cdu_parameters.standby_channel, 0, TACAN_S);
		memcpy(cdu_parameters.standby_channel, cdu_parameters.p1_channel,
				strlen(cdu_parameters.p1_channel));
		break;
	case '2':
		memset(cdu_parameters.standby_channel, 0, TACAN_S);
		memcpy(cdu_parameters.standby_channel, cdu_parameters.p2_channel,
				strlen(cdu_parameters.p2_channel));
		break;
	case '3':
		memset(cdu_parameters.standby_channel, 0, TACAN_S);
		memcpy(cdu_parameters.standby_channel, cdu_parameters.p3_channel,
				strlen(cdu_parameters.p3_channel));
		break;
	case '4':
		memset(cdu_parameters.standby_channel, 0, TACAN_S);
		memcpy(cdu_parameters.standby_channel, cdu_parameters.p4_channel,
				strlen(cdu_parameters.p4_channel));
		break;
	case '5':
		memset(cdu_parameters.standby_channel, 0, TACAN_S);
		memcpy(cdu_parameters.standby_channel, cdu_parameters.p5_channel,
				strlen(cdu_parameters.p5_channel));
		break;
	case '6':
		memset(cdu_parameters.standby_channel, 0, TACAN_S);
		memcpy(cdu_parameters.standby_channel, cdu_parameters.p6_channel,
				strlen(cdu_parameters.p6_channel));
		break;
	case '7':
		memcpy(cdu_parameters.standby_channel, cdu_parameters.p7_channel,
				strlen(cdu_parameters.p7_channel));
		break;
	case '8':
		memset(cdu_parameters.standby_channel, 0, TACAN_S);
		memcpy(cdu_parameters.standby_channel, cdu_parameters.p8_channel,
				strlen(cdu_parameters.p8_channel));
		break;
	default:
		break;
	}
...
bool decode_message(uint8_t *rx_buffer, Identifier *ident, uint8_t *mhz,
		uint16_t *khz, uint8_t *f_flag, uint8_t *vol, uint8_t *mode) {
...
//    uint8_t khz_length = 0;
	...
	if (received_msg[len - 2] == '\r' || received_msg[len - 1] == '\n') {
		if (strncmp(received_msg, "$PATC", 5) != 0
				&& strncmp(received_msg, "$PMRR", 5) != 0
				&& strncmp(received_msg, "$PATN", 5) != 0) {
	...
	if ((ident->msg_id == S_FREQ) || (ident->msg_id == A_FREQ)) {
		*f_flag = true;
		...
		if ((ident->class == NAV) || (ident->class == VHF)
				|| (ident->class == TACAN) || (ident->class == UHF)) {
			char mhz_c, khz_c;
...
//                    khz_length = i+1;
	...
	if (ident->msg_id == MODE) {
		char mode_tcan = 0;
		if (sscanf(&received_msg[8], "%c", &mode_tcan) == 1) // Check for successful parsing
				{
			*mode = mode_tcan - CONVERSION_VAL_DEC; // Adjust the volume as needed
		}
	}
	...
	char cal_sum[15];
	memset(cal_sum, 0, 15);
	strncpy(cal_sum, &received_msg[6], len - 10);  // Extract the checksum
...
void decode_receive_data(uint8_t *buffer) {
	...
	if (decode_success) {
		if (mode < 2) {
			notify_tft_tacan(mode);
		}
		message_parse[ident.class](&ident, mhz, khz, vol);
		if (ident.msg_id != Health) {
			notify_tft();
			page_class = ident.class;
			messag_id = ident.msg_id;
		}
	}
}
...
void encode_message(uint8_t *tx_buffer, Class_Id c_index, Message_Id m_index,
	...
	if ((f_flag == 'Y') && (c_index != ADF) && (c_index != HF)) {
		mhz -= CONVERSION_VAL_DEC;
		khz = (khz / 25);
		khz += CONVERSION_VAL_DEC;
		char khz_ascii = (char) khz;
		if ((c_index == NAV) || (c_index == VHF)) {
			sprintf(msg, "%s%c%cN", message_id_nav_vhf[m_index],
					(unsigned char) mhz, khz_ascii);
		} else if ((c_index == TACAN) || (c_index == UHF)) {
			sprintf(msg, "%s%c%cN", message_id_adf_tacan[m_index],
					(unsigned char) mhz, khz_ascii);
		}
	} else if ((f_flag == 'Y') && ((c_index == ADF) || (c_index == HF))) {
		mhz += CONVERSION_VAL_DEC;
		uint8_t split_khz[5];
		uint8_t size = split_frequency_khz(khz, split_khz);
		split_khz[size] = 0;
		sprintf(msg, "%s%c%sN", message_id_adf_tacan[m_index],
				(unsigned char) mhz, split_khz);
	} else if ((f_flag == 'N') && (m_index == VOLUME)) {
		if ((c_index == NAV) || (c_index == VHF)) {
			sprintf(msg, "%s%c", message_id_nav_vhf[m_index], (vol + 48));
		} else if ((c_index == ADF) || (c_index == TACAN)) {
			sprintf(msg, "%s%c", message_id_adf_tacan[m_index], (vol + 48));
		}
	} else if ((f_flag == 'N') && (m_index == MODE)) {
		if (c_index == TACAN) {
			sprintf(msg, "%s%c", message_id_adf_tacan[m_index], (mode + 48));
		}
	} else {
		if ((c_index == NAV) || (c_index == VHF)) {
			sprintf(msg, "%s", message_id_nav_vhf[m_index]);
		} else if ((c_index == ADF) || (c_index == TACAN) || (c_index == HF)
				|| (c_index == UHF)) {
...
void handle_ADF_incoming_message(Identifier *ident, uint8_t f_mhz,
	...
	case S_FREQ:
		// get the frequency and update standby
		cdu_parameters.standby_adf_freq = (uint16_t) (f_mhz * 1000) + f_khz;
		break;
	case A_FREQ:
		// get the frequency and update active
		cdu_parameters.active_adf_freq = (uint16_t) (f_mhz * 1000) + f_khz;
		break;
	case VOLUME:
		// update Volume
		cdu_parameters.volume_adf = vol;
		break;
	case RESET_STATUS:
		// update status
		break;
...
void handle_VHF_incoming_message(Identifier *ident, uint8_t f_mhz,
	...
	case S_FREQ:
		// get the frequency and update standby
		freq = get_freq(f_mhz, f_khz);
		status = input_data_validation_freq(VHF, freq);
		if (status) {
			cdu_parameters.standby_vhf_freq = freq;
		}
		break;
	case A_FREQ:
		// get the frequency and update active
...
//			freq = get_freq(f_mhz, f_khz);
//			status = input_data_validation_freq(TACAN, freq);
//			if(status)
//			{
//				cdu_parameters.standby_tacan_freq = freq;
//			}
		break;
	case A_FREQ:
...
//			if(status)
//			{
//				cdu_parameters.active_tacan_freq = freq;
//			}
		break;
	case VOLUME:
		// update Volume
		cdu_parameters.volume_tacan = vol;
		break;
	case RESET_STATUS:
		// update status
		break;
...
void notify_tft_tacan(tacan_mode mode) {
	...
	char key;
	key = mode ? SWI_FREQUENCY : SWI_CHANNEL;
	if (xKeyQueue != NULL) {
		xQueueSend(xKeyQueue, &key, portMAX_DELAY);
	}
}
